...
<?php
	...
	if (isset($_POST['select']) && ($_POST['select'])) {    
		...
		// Kiểm tra xem mục đã chọn đã tồn tại trong phiên hay chưa
		$monAnTonTai = false;
		if (isset($_SESSION['monchinh'])) {
			foreach ($_SESSION['monchinh'] as $monan) {
				// Giả sử $monan[0] là chỉ số của 'tenmon' trong mảng của bạn
				if ($tenmon == $monan[0]) {
					$monAnTonTai = true;
					break;
				}
		...
		if ($monAnTonTai) {
			echo '<script>alert("Món ăn đã có trong thực đơn!");</script>';
		} else {
                    ...
                    <form id="" name="" method="post" action="">
                      ...
                      <?php
                          // Lấy ngày hiện tại
							...
							// Kiểm tra xem ngày hiện tại có phải là Chủ Nhật (Sun) hay không
							//if ($currentDate === 'Sun' || $currentDate === 'Sat') {
								// Nếu là Chủ Nhật, hiển thị nút "Thêm thực đơn"
								switch($_POST['nut'])
								{
									case 'Thêm thực đơn':
									{
							...
							//} else {
								// Nếu không phải là Chủ Nhật, không hiển thị nút
								/*echo '<script>alert("Ngày hiện tại không phải là cuối tuần, không thể thêm thực đơn.");</script>';
								echo'<script>
														window.location="thucdon.php";
													</script>';
							}
