                ...
                switch($_REQUEST['nut']){
                    case 'Thêm':{
                        if(isset($_SESSION['user'])){
                            $user=$_SESSION['user'];
                            $Ngaydat = $_SESSION['ngay'] ;
                            $now = time();
							$future_date = strtotime($_SESSION['ngay']);
							$time_diff = $future_date - $now;
							if($time_diff > 1)
							{
                                if(isset($_REQUEST['soluong'])) $soluong=$_REQUEST['soluong'];
								$check="select * from giohang where Username='$user' and MaMon='$mamon' and Ngaydat = '$Ngaydat' and TrangThai=0";
								if($p->kiemtragiohang($check)==1){
									$sql="update giohang set SoLuong=SoLuong+$soluong , Ngaydat = '$Ngaydat' where Username='$user' and MaMon='$mamon'";
									if($p->themsuagiohang($sql)==1){
										echo '<script language="javascript">alert("Cập nhật giỏ hàng thành công");</script>';
										echo '<script language="javascript">window.location.href="thucdon-user.php"</script>';
									}
									else{
									...
									{
										echo '<script language="javascript">alert("Ngày đặt phải hơn 24h so với hiện tại. Vui lòng chọn lại trong thực đơn khác.");</script>';
										echo '<script language="javascript">window.location.href="thucdon-user.php"</script>';	
									}
